...
class GoogleMapAPI{
	...
	/** @var string  Ratio 16:9 ( 100%/16*9 = 56.25% ) * */
	protected $responsive = true;
	protected $ratio = '56.25%';
	...
	public function geocoding($address){
		...
		$url = "https://maps.google.com/maps/api/geocode/json?address={$address}&key={$this->ApiKey}";
	...
	public function init(){
		...
		if ($this->responsive == true){
			$this->content .= "\t" . '<div id="' . $this->googleMapId . '" class="r_map" style="width:' . $this->width . ';height:' . $this->height . '"></div>' . "\n";
			$this->content .= "\t <style>
				.r_map{ position: relative; width: 100%!important; padding-bottom:$this->ratio;  overflow: hidden!important; }
 				</style> \n";
		...
		if ($this->includeJs === TRUE){
			// Google map JS
			$this->content .= '<script type="text/javascript" src="https://maps.googleapis.com/maps/api/js?key=' . $this->ApiKey . '&language=' . $this->lang . '">';
			$this->content .= '</script>' . "\n";
